...
AddrRewriter(INPUTSPEC1, ..., INPUTSPECn)
...
their checksums.  AddrRewriter implements the functionality of a network
...
AddrRewriter maintains a I<mapping table> that records how addresses are
rewritten.  On receiving a packet, AddrRewriter first looks up that packet
...
address must be a dash '-', since AddrRewriter only changes outgoing
...
AddrRewriter-like pattern.

=back

Input packets must have their IP header annotations set.  AddrRewriter
...
Returns a human-readable description of the AddrRewriter's current mapping
table.
...
class AddrRewriter : public RewriterBase { public:
    ...
    AddrRewriter() CLICK_COLD;
    ~AddrRewriter() CLICK_COLD;

    const char *class_name() const		{ return "AddrRewriter"; }
...
inline void
AddrRewriter::destroy_flow(RewriterFlow *flow)
{
