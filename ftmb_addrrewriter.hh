...
FTMBAddrRewriter(INPUTSPEC1, ..., INPUTSPECn)
...
their checksums.  FTMBAddrRewriter implements the functionality of a network
...
FTMBAddrRewriter maintains a I<mapping table> that records how addresses are
rewritten.  On receiving a packet, FTMBAddrRewriter first looks up that packet
...
address must be a dash '-', since FTMBAddrRewriter only changes outgoing
...
FTMBAddrRewriter-like pattern.

=back

Input packets must have their IP header annotations set.  FTMBAddrRewriter
...
Returns a human-readable description of the FTMBAddrRewriter's current mapping
table.
...
/// FTMBAddrRewriter works as follows:
...
/// On receiving a packet, FTMBAddrRewriter first looks up _map.
...
class FTMBAddrRewriter : public RewriterBase { public:
    ...
    FTMBAddrRewriter() CLICK_COLD;
    ~FTMBAddrRewriter() CLICK_COLD;

    const char *class_name() const		{ return "FTMBAddrRewriter"; }
...
inline void
FTMBAddrRewriter::destroy_flow(RewriterFlow *flow)
{
