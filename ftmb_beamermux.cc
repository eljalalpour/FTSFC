...
void FTMBBeamerMux::_init_shared_state() {
    Router *r = this->router();
    _shared_state = (FTMBSharedStateBeamer *)(r->find(_shared_state_element_name));
}

int FTMBBeamerMux::configure(Vector<String> &conf, ErrorHandler *errh) {
    int ringSize = 1;
    int maxStates = -1;
...
Packet *FTMBBeamerMux::simple_action(Packet *p) {
