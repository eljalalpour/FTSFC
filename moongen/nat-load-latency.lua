...
-- set addresses here
local SRC_MAC = "0c:c4:7a:73:fa:dc"
local DST_MAC = "0c:c4:7a:73:fa:18"
...
function configure(parser)
    ...
    parser:option("-o --out", "Output folder."):default("./")
    parser:option("-s --size", "Packet size."):default(1000):convert(tonumber)
    parser:option("-d --duration", "The duration of a period (in seconds)"):default(10):convert(tonumber)
    parser:option("-p --periods", "Number of periods (in seconds)"):default(6):convert(tonumber)
end
...
function master(args)
    ...
    local total_time = args.periods * args.duration
...
function timerSlave(txQueue, rxQueue, size, periods, duration, hist_out, overtime_out)
    ...
    local totalDurTimeout = timer:new(periods * duration)

    while mg.running() and totalDurTimeout:running() do
