...
def ft_block_names_list(thrds):
    """
    the list of TF block names
    :param thrds: a number denoting the number of threads
    :return: names list
    """
    return [TF_BLOCK_NAME_FORMAT_STR.format(i) for i in range(thrds)]
...
def ft_blocks_declares(chain_pos, ch_len, thrds, mb):
    ...
    for thrd in range(thrds):
        ...
        declares.append(
            TF_BLOCK_FORMAT_STR.format(**params)
        )
...
def links(ch_len, chain_pos, thrds):
    ...
    for i in range(thrds):
        ll.append(LINK_FORMAT_STR.format(**{
            ...
            'TF_BLOCK_NAME': ft_block_names[i],
...
def tf_click(ch_len, chain_pos, thrds, mb, sharing_level, f, batch):
    ...
    """
    string_map = {
        SHARED_STATE_DECLARE: shared_state_declare(thrds,
                                                   mb,
                                                   sharing_level,
                                                   batch,
                                                   f),
        ...
        TF_BLOCKS_DECLARES: ft_blocks_declares(chain_pos,
                                                 ch_len,
                                                 thrds,
                                                 mb),
...
def store(output_path, clicks):
    for i in range(len(clicks)):
        file_name = os.path.join(output_path, TF_OUTPUT_NAME_FORMAT_STR.format(i + 1))
